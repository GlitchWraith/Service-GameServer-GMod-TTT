...
ARG collection=1780079141
ARG steamAPIKey

RUN useradd -m steam
RUN apt-get update && apt-get install -y --no-install-recommends lib32gcc1 wget gdb gcc-multilib
...
RUN buildWorkaround.sh $collection $steamAPIKey

ENV liveCollection=collection
ENV liveAPIKey
ENTRYPOINT [ "/bin/bash", "-c", "/home/steam/Server1/srcds_run -game garrysmod +maxplayers 12 +map gm_flatgrass +ga memode terrortown -console +host_workshop_collection $liveCollection -authkey $liveAPIKey -debug" ]
